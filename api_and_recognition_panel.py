...
class APIAndRecognitionPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.current_screenshot_shortcut = "printscreen"  # 改为printscreen
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # 连接信号
        self.buttons_panel.select_image_requested.connect(self.select_image_requested.emit)
        self.buttons_panel.take_screenshot_requested.connect(self.take_screenshot_requested.emit)
        self.buttons_panel.retry_recognition_requested.connect(self.retry_recognition_requested.emit)
        self.buttons_panel.compact_mode_toggled.connect(self.compact_mode_toggled.emit)
        
        # 连接截图快捷键按钮
        self.buttons_panel.shortcut_btn.clicked.connect(self.show_screenshot_shortcut_dialog)
    ...
    def get_api_config_manager(self):
        """获取API配置管理器"""
        return self.config_panel.get_api_config_manager()
